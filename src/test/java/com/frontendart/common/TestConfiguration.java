 ...
 *
 * The following steps should be done after each testsite update: - the email address of the mtmtuser3 user should be approved
 *
 *
 * @author Zoli
 *
...
// java -jar "selenium-server-standalone-2.53.0.jar" -Dwebdriver.firefox.bin="/home/zalan/ff46/firefox/firefox"
    ...
    private static void createGui2AuthenticatedUrl() throws URISyntaxException {
        ...
        URI authenticatedServerUrl = new URI(serverUrl.getScheme(),
                String.format("%s:%s@%s", gui2Username, gui2Password, serverUrl.getHost()), serverUrl.getPath(), serverUrl.getQuery(),
                serverUrl.getFragment());
