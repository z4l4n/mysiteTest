...
public enum MainQueryLocators implements GeneralLocatorTypes {
	...
	FILTER_INPUT_FIELD(SMART_QUERIES_GRIDPANEL + "//*/div[starts-with(@id, 'filterwidget')]//*/input",
			Arrays.asList("Filter mező", "Filter field")),
	FILTER_BUTTON(SMART_QUERIES_GRIDPANEL + "//*/a[@data-qtip='Rekordok szűkítése' or "
			+ "@data-qtip='Filter records']", Arrays.asList("Szűkítés", "Filter")),
	MY_QUERIES_CHECKBOXFIELD("(" + SMART_QUERIES_GRIDPANEL + "//div/div/div/span[starts-with(@id, 'checkbox')])[4]",
			Arrays.asList("Saját keresések checkbox", "My own queries checkbox")),
	NEW_QUERY_BUTTON(SMART_QUERIES_GRIDPANEL + "//span[text()='Új keresés' or text()='New search']", Arrays.asList("Új keresés", "New query")),
	...
	SAVE_AS_LIST("./../../div//span[text()='Eredmények mentése listaként' or text()='Save results as list']", Arrays.asList("Mentés listaként", "Save results as list")),
	DUPLICATE("./../../div//span[text()='Másolás' or text()='Copy']", Arrays.asList("Másolás", "Copy")),
	DELETE("./../../div//span[text()='Töröl' or text()='Delete']", Arrays.asList("Töröl", "Delete")),
	SAVE_AS_DEFAULT("./../../div//span[text()='Beállítás alapértelmezettként' or text()='Set as default']", Arrays.asList("Beállítás alapértelmezettként", "Set as default")),
