...
public enum RecordEditorLocators implements GeneralLocatorTypes {
    NEW_BUTTON(
            "//a[starts-with(@data-qtip, 'Új rekord felvitele') or "
                    + "starts-with(@data-qtip, 'Add new record')]",
            Arrays.asList("Új", "New")),
    EDIT_BUTTON(
            "//a[starts-with(@data-qtip, 'Rekord szerkesztése') or "
                    + "starts-with(@data-qtip, 'Edit record')]",
            Arrays.asList("Szerkeszt", "Edit")),
    REMOVE_BUTTON(
            "//a[starts-with(@data-qtip, 'Rekord törlése') or "
                    + "starts-with(@data-qtip, 'Delete record')]",
            Arrays.asList("Törlés", "Delete")),

    OPERATIONS(
            "//a[starts-with(@data-qtip, 'Egyéb műveletek') or "
                    + "starts-with(@data-qtip, 'Other Operations')]",
            Arrays.asList("Egyéb műveletek", "Other Operations")),
    ...
    ENABLE_USER(
            OPERATIONS
                    + "//div[contains(@id, 'enableusermenuitem-')]/a[starts-with(@id, 'enableusermenuitem-')]/span[text()='Felhasználó belépésének engedélyezése']"),
    DISABLE_USER(
            OPERATIONS
                    + "//div[contains(@id, 'disableusermenuitem-')]/a[starts-with(@id, 'disableusermenuitem-')]/span[text()='Felhasználó belépésének tiltása']"),

    CHANGE_STATE_COMBOBOX_PICKER(
            "//div[starts-with(@id, 'changestatecombobox-') and contains(@id, '-trigger-picker')]",
            Arrays.asList("Státusz-választó lenyitó gomb", "Change state picker")),
    ...
    CHANGE_STATE_ADMIN_APPROVED(
            "//li[contains(@class, 'combo-status-admin_approved')]",
            Arrays.asList("Admin láttamozott", "Admin approved")),
    ...
    EDITOR_FORM_VISIBLE_FIELDS(
            EDITOR_FORM + "//div[contains(@class, ' x-form-item x-form-item-default ')][not(contains(@style, 'display: none'))]"),
    ...
    CLOSE(
            MODEL_EDITOR_WINDOW + "//div[(@data-qtip='Ablak bezárása' or @data-qtip='Close window')]//div",
    ...
    SWORD_WINDOW_HEADER(
            "//div[starts-with(@id, 'sworduploadwindow')][starts-with(@class, 'x-window-header')]",
            Arrays.asList("SWORD feltöltés", "SWORD upload")),
    SWORD_WINDOW_CLOSE(SWORD_WINDOW_HEADER + "//img", Arrays.asList("SWORD feltöltés bezárás", "SWORD upload close")),

    PUBLICATION_SAVE_WINDOW(
            EDITOR_WINDOW + "//div[starts-with(@id, 'PublicationSaveMEF-') and contains(@id, '-bodyWrap')]",
            Arrays.asList("Közlemény szerkesztő ablak", "Publication editor window")),
    PUBLICATION_EDITOR_WINDOW(
            MODEL_EDITOR_WINDOW + "//div[starts-with(@id, 'PublicationSaveMEF-') and contains(@id, '-bodyWrap')]",
            Arrays.asList("Közlemény szerkesztő ablak", "Publication editor window")),
    PUBLICATION_SAVE_AND_CLOSE_BUTTON(
            PUBLICATION_SAVE_WINDOW
                    + "//*/span[text()='Mentés és bezárás' or text()='Save and close' or text()='Csatol & bezár' or text()='Connect & close']",
            Arrays.asList("Mentés és bezárás", "Save and close")),

    KEYWORD_EDITOR_WINDOW(
            MODEL_EDITOR_WINDOW + "//div[starts-with(@id, 'GenericMEF-') and contains(@id, '-bodyWrap')]",
            Arrays.asList("Kulcsszó szerkesztő ablak", "Keyword editor window")),
    ...
    public static boolean checkAttributesAreVisible(final Class<? extends GeneralTableAttributes> pageToValidate) {
        ...
        for (final GeneralTableAttributes locator : pageToValidate.getEnumConstants()) {
            if ((!locator.getAttributeFlags().contains(RecordAttributeFlags.DISABLED))
                    && (!Utils.doesThisStringListContainsThisAttribute(labelsAsString, locator))) {
                returnValue = false;
            }
        }
