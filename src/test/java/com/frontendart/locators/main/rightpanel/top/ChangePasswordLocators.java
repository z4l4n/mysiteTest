...
public enum ChangePasswordLocators implements GeneralLocatorTypes {
    ...
    USERNAME_SEARCH_TRIGGER(
            MODEL_EDIT_WINDOW + "//div[contains(@id, 'trigger-search')]",
            Arrays.asList("Felhasználó keresés ikon", "Username search icon")),
    ...
    PASSWORD_AGAIN_FIELD(
            MODEL_EDIT_WINDOW + "//input[starts-with(@id, 'passwordfield2-')]",
            Arrays.asList("Új jelszó újra", "New password again")),
    MODEL_EDIT_WINDOW_CANCEL_BUTTON(
            MODEL_EDIT_WINDOW + "//a[contains(@class, 'x-btn-default-toolbar-small')]//span",
    ...
    USERNAME_FILTER_BUTTON(
            "//div[starts-with(@id, 'selectorbox')]//span[text()='Szűkítés' or text()='Filter']",
            Arrays.asList("Szűrés", "Filter")),
    USERNAME_FILTER_SAVE(
            "//div[starts-with(@id, 'selectorbox')]//span[text()='Kiválaszt és bezár' or text()='Select and close' or text()='Csatol & bezár' or text()='Connect & close']",
            Arrays.asList("Mentés és bezárás", "Save and close")),
    USERNAME_FILTER_FIRST_MATCH(
            "//div[starts-with(@id, 'selectorbox')]//div[starts-with(@class, 'x-grid-item-container')]//table[1]/tbody/tr/td/div",
            Arrays.asList("Első találat", "First match")),
    USERNAME_FILTER_NO_RESULTS("//div[(@class='x-grid-empty')]", Arrays.asList("Nincs találat", "No Results")),
    USERNAME_FILTER_CLOSE_BUTTON(
            USERNAME_WINDOW + "//a[contains(@class, 'x-btn-default-toolbar-small')]//span",
    ...
    ADMIN_FORUM_TAB(
            "//span[contains(text(), 'Admin fórum') or contains(text(), 'Admin forum')]",
            Arrays.asList("Admin fórum", "Admin forum")),
    INSTADMIN_FORUM_TAB("//span[text()='Intézményi fórum' or text()='Institute forum']", Arrays.asList("Intézményi fórum", "Admin forum")),

    // TODO: remove from here
    ERROR_WINDOW_AFTER_LOGIN(
            "//div[starts-with(@id, 'window')]//div[starts-with(@id, 'title')][text()='Figyelem']",
            Arrays.asList("Figyelem", "Attention")),
