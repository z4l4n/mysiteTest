...
public enum InstituteRecordAttributes implements GeneralTableAttributes {
	...
	ABBREVIATION_ENG(Arrays.asList("Rövidítés angolul", "Abbreviation En"), FieldTypeLocators.TEXTFIELD, Arrays.asList(RecordAttributeFlags.REQUIRED)),
	INSTITUTE_TYPE(Arrays.asList("Típus", "Type"), FieldTypeLocators.RECORDSELECTORFIELD, Arrays.asList(RecordAttributeFlags.REQUIRED, RecordAttributeFlags.EXTENDABLE)),
	ONLINE_REGISTRATION(Arrays.asList("Online regisztráció", "Online registration"), FieldTypeLocators.CHECKBOXFIELD, Arrays.asList(RecordAttributeFlags.REQUIRED, RecordAttributeFlags.EXTENDABLE)),
	OFFLINE_REGISTRATION_GUIDE(Arrays.asList("Offline regisztrációs útmutató", "Offline registration guide"), FieldTypeLocators.TEXTAREAFIELD, Arrays.asList(RecordAttributeFlags.REQUIRED));
	 ...
	 */
	private InstituteRecordAttributes(final List<String> names, final FieldTypeLocators fieldType, final List<RecordAttributeFlags> attributeFlags) {
		this.names = names;
