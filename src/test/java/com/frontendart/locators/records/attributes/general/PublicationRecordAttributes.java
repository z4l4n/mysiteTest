...
public enum PublicationRecordAttributes implements GeneralTableAttributes {
    ...
    LANGUAGE(
            ...
            Arrays.asList(RecordAttributeFlags.REQUIRED, RecordAttributeFlags.EXTENDABLE)),
    AUTHORSHIP(
            Arrays.asList("Szerzőségek", "Authorships"),
            FieldTypeLocators.RECORDSELECTORLISTFIELD,
            Arrays.asList(RecordAttributeFlags.REQUIRED, RecordAttributeFlags.EXTENDABLE)),
    TITLE(Arrays.asList("Cím", "Title"), FieldTypeLocators.TEXTAREAFIELD, Arrays.asList(RecordAttributeFlags.REQUIRED)),
    SUBTITLE(Arrays.asList("Alcím", "Sub Title"), FieldTypeLocators.TEXTAREAFIELD, Arrays.asList(RecordAttributeFlags.DISABLED)),
    PUBLISHED_YEAR(
            Arrays.asList("Megjelenés éve", "Published Year"),
            FieldTypeLocators.NUMBERFIELD,
            Arrays.asList(RecordAttributeFlags.REQUIRED, RecordAttributeFlags.NOT_SEARCHABLE)),
    SUBJECTS(
            Arrays.asList("Tudományterület", "Subjects"),
            FieldTypeLocators.RECORDSELECTORLISTFIELD,
            Arrays.asList(RecordAttributeFlags.REQUIRED, RecordAttributeFlags.EXTENDABLE)),
    NUMBER_OF_CITATIONS(
