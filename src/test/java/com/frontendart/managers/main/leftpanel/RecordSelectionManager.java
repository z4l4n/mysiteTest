...
public class RecordSelectionManager {
	...
	/**
	 * Finds and clicks record type in selector
	 * @throws  
	 */
	public static boolean findThisTypeInSelectorOptionsAndClick(final GeneralRecordTypes recordType)  {
		...
		try {
			...
			final WebElement selectThis = Utils.createGeneralWebElementFromString("//div[starts-with(@id, 'menu-')][contains(@class, 'cycle')]//span[starts-with(@id, 'menucheckitem-')][text()='" + text + "']");
...
//			}
	...
	public static boolean selectThisRecordTypeFromSelector(final GeneralRecordTypes recordType) {
		LOGGER.info("Válasszuk ki az alábbi rekord típust a rekord választóból: " + recordType.getNames().get(0) + "/" + recordType.getNames().get(1));
...
/**
	...
	public static List<WebElement> selectRandomRenderedRowsFromGridPanel(final int count) {
		final List<WebElement> selectedRowWebElements = new ArrayList<WebElement>();
...
//					gridTableFullSize() > 1);
	...
	public static void controlClickOnThisRow(final WebElement rowToClick) {
		...
		builder.keyDown(Keys.CONTROL).click(rowToClick.findElement(By.xpath(RecordSelectionLocators.RECORD_SELECTION_SELECT_ROW.toString()))).keyDown(Keys.CONTROL);
//		builder.keyDown(Keys.CONTROL).click(rowToClick).keyDown(Keys.CONTROL);
	...
	public static void shiftClickOnThisRow(final WebElement startRow, final WebElement endRow) {
		...
		builder.click(startRow.findElement(By.xpath(RecordSelectionLocators.RECORD_SELECTION_SELECT_ROW.toString()))).
				keyDown(Keys.SHIFT).click(endRow.findElement(By.xpath(RecordSelectionLocators.RECORD_SELECTION_SELECT_ROW.toString()))).keyDown(Keys.SHIFT);
		final Action selectMultiple = builder.build();
		selectMultiple.perform();
		Utils.waitMillisec(2000);
	}
	...
	public static List<WebElement> getSelectedItemsInView() {
		final List<WebElement> selectedElements = new ArrayList<WebElement>();
