...
public class RecordSelectionManager {
	...
	// Language check (If English is the active, we should check for the English
	// name of the attribute)

	final String text = ("English"
		.equals(Utils.createGeneralWebElementFromEnum(MainPageLocators.CHANGE_LANGUAGE_BUTTON).getText()))
	...
	if ((activeElementText.equals(recordType.getNames().get(0)))
		|| activeElementText.equals(recordType.getNames().get(1))) {
	    activeElement.click();
	    returnValue = true;
	} else {
    ...
    /**
     * Selects a record from the record selector and run a default query to have
     * results
     */
	...
	// org.junit.Assume.assumeFalse("There are no " +
	// getAllMyLabelsAsString(recordType) + " type records present!",
	// 0==gridTableFullSize());
	// assertNotEquals("There are no " + getAllMyLabelsAsString(recordType) + " type
	// records present!", 0, gridTableFullSize());
	// }
	...
	if (Utils.createGeneralWebElementFromEnum(ToolbarLocators.CHANGE_VIEW_GRID).getAttribute("class")
		.contains("pressed")) {
	    Utils.waitForElementPresent(MainPageLocators.CONTENT_GRID_PANEL);
	} else if (Utils.createGeneralWebElementFromEnum(ToolbarLocators.CHANGE_VIEW_LISTGRID).getAttribute("class")
		.contains("pressed")) {
	...
	final GeneralRecordTypes[] records = new GeneralRecordTypes[] { GeneralRecordTypes.APPEARANCE,
		GeneralRecordTypes.AUTHOR_TYPE, GeneralRecordTypes.CATEGORY, GeneralRecordTypes.DEGREE,
		GeneralRecordTypes.FORUM, GeneralRecordTypes.FUNDER, GeneralRecordTypes.INSTITUTE_TYPE,
		GeneralRecordTypes.LANGUAGE, GeneralRecordTypes.RATING_TYPE, GeneralRecordTypes.SUB_TYPE,
	...
	final String myInfoText = Utils.createGeneralWebElementFromEnum(RecordSelectionLocators.INFO_BAR_NUMBERS)
		.getText();
	...
	LOGGER.info("Válasszunk ki " + count
		+ "db sort a táblázatból. (Jelen tesztben kiválasztott sor, sorok sorszáma: " + selectedRows + ")");
	...
	// Logging
	LOGGER.info(
		"Kattintsunk tetszőleges sorokra a táblázatban úgy, hogy a CTRL (Control) gombot lenyomva tartjuk. ");
	LOGGER.info(
		"Elvárt eredmény: Ha az adott (kattintott) sor már ki volt jelölve, akkor a kattintás következtében "
	...
	for (int index = 0; index < howManyRowsToClickOn; index++) {
	    ...
	    // If the selected row was already in the selected rows list, we remove,
	    // otherwise we put it
	...
	if (gridTableFullSize() < 2) {
	    ...
	    assumeTrue("There are not enough records to perform this test case! At least 2 records should be present!",
		    gridTableFullSize() > 1);
	    // assertTrue("There are not enough records to perform this test case! At least
	    // 2 records should be present!",
	    // gridTableFullSize() > 1);
	}
	...
	// Logger
	LOGGER.info(
		"Válasszunk egy tetszőleges sort a táblázatból, kattintsunk rá, majd a SHIFT gombot nyomva tartva válasszunk "
	...
	// Logger
	LOGGER.info(
		String.format("Válasszuk ki a legnagyobb id-jű %d db sort a táblázatból a SHIFT gombot lenyomásával.",
			numberOfSelections));
	...
	final Actions builder = Utils.createActionForDriver();
	builder.click(startRow.findElement(By.xpath(RecordSelectionLocators.RECORD_SELECTION_SELECT_ROW.toString())))
		.keyDown(Keys.SHIFT)
		.click(endRow.findElement(By.xpath(RecordSelectionLocators.RECORD_SELECTION_SELECT_ROW.toString())))
		.keyDown(Keys.SHIFT);
	final Action selectMultiple = builder.build();
	selectMultiple.perform();
	Utils.waitMillisec(2000);
