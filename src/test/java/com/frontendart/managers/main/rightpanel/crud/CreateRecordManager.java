	...
	public static WebElement findFieldOfThisAttribute(final GeneralTableAttributes attribute) {
		...
		try {
			...
			try {
				...
				fail("Could not find this attribute on page: " + Utils.getAllMyLabelsAsString(attribute) + ". Error message is: " + exc.getMessage());
			}
		}
...
//		}
	...
	public static void checkSuccessfulRecordCreationMessage() {
		...
		case AUTHOR:
			assertTrue("There should be a message present on the new record creation page!", Utils.isMessageBoxPresentWithText(RecordEditorMessageTypes.AUTHOR_MESSAGE));
			break;
		case INSTITUTIONAL_ADMIN:
			assertTrue("There should be a message present on the new record creation page!", Utils.isMessageBoxPresentWithText(RecordEditorMessageTypes.INSTADMIN_MESSAGE));
			break;
		default:
			break;
		}
