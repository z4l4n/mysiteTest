...
public class ChangeViewManager {
    ...
    public static void switchToListGridView() {
        if (!Utils.createGeneralWebElementFromEnum(ToolbarLocators.CHANGE_VIEW_LISTGRID).getAttribute("class").contains("x-btn-pressed")) {
    ...
    public static void switchToGridView() {
        if (!Utils.createGeneralWebElementFromEnum(ToolbarLocators.CHANGE_VIEW_GRID).getAttribute("class").contains("x-btn-pressed")) {
