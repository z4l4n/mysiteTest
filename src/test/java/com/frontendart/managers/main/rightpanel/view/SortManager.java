...
public class SortManager {
	...
	public static void checkNumberOfSorters(final int expectedNumber) {
		...
		if (expectedNumber < 3) {
			Utils.myAssertFalse("A " + SortLocators.NEW_SORTING_BUTTON.getNames().get(0) + "/" + SortLocators.NEW_SORTING_BUTTON.getNames().get(1)
					+ " nevű gombnak aktívnak kell lennie.",
					Utils.createGeneralWebElementFromEnum(SortLocators.NEW_SORTING_BUTTON).getAttribute("class").contains("x-item-disabled"));
		} else {
			Utils.myAssertTrue("A " + SortLocators.NEW_SORTING_BUTTON.getNames().get(0) + "/" + SortLocators.NEW_SORTING_BUTTON.getNames().get(1)
	...
	public static void doCompare(final List<Integer> attributePositions) {
		...
		for (int index = 0; index < (myGridPanelRows.size() - 1); index++) {
			...
			final String firstValueOfThisRow = myGridPanelRows.get(index).findElement(By.xpath("./tbody[1]/tr/td[" + attributePositions.get(0) + "]")).getText();
			final String firstValueOfNextRow = myGridPanelRows.get(index + 1).findElement(By.xpath("./tbody/tr[1]/td[" + attributePositions.get(0) + "]")).getText();
			...
			if ((firstValueOfThisRow.equals(firstValueOfNextRow)) && (getNumberOfSorters() > 1)) {
				...
				assertTrue("Comparing by first attribute: " + firstValueOfThisRow + " <= " + firstValueOfNextRow, myCollator.compare(firstValueOfThisRow, firstValueOfNextRow) <= 0);
			}
		}
	}
	...
	public static void compareBySecondAttribute(final List<Integer> attributePositions, final int index) {
		...
		final String secondValueOfThisRow = myGridPanelRows.get(index).findElement(By.xpath("./tbody[1]/tr/td[" + attributePositions.get(1) + "]")).getText();
		final String secondValueOfNextRow = myGridPanelRows.get(index + 1).findElement(By.xpath("./tbody/tr/td[" + attributePositions.get(1) + "]")).getText();
		...
		if ((secondValueOfThisRow.equals(secondValueOfNextRow)) && (getNumberOfSorters() > 2)) {
			...
			assertTrue("Comparing by second attribute: " + secondValueOfThisRow + " <= " + secondValueOfNextRow, myCollator.compare(secondValueOfThisRow, secondValueOfNextRow) <= 0);
		}
	}
	...
	public static void compareByThirdAttribute(final List<Integer> colPositions, final int index) {
		...
		final String thirdValueOfThisRow = myGridPanelRows.get(index).findElement(By.xpath("./tbody/tr/td[" + colPositions.get(2) + "]")).getText();
		final String thirdValueOfNextRow = myGridPanelRows.get(index + 1).findElement(By.xpath("./tbody/tr/td[" + colPositions.get(2) + "]")).getText();
	...
	public static List<GeneralTableAttributes> createRandomSorterForThisRecordType(final GeneralRecordTypes recordType) {
		...
		final List<GeneralTableAttributes> selectedAttributes = new ArrayList<GeneralTableAttributes>();
	...
	public static void sortTableByIdDesc() {
		...
		Utils.createGeneralWebElementsFromEnum(SortLocators.ALL_SORTERS).get(0).findElement(By.xpath(SortLocators.SORTER_ARROW.toString())).click();
	...
	public static void selectThisAttributeFromSorter(final GeneralTableAttributes attribute) {
		...
		// Get menu options
		WebElement activeElement = Utils.createGeneralWebElementsFromEnum(SortLocators.SORTER_MENU_OPTIONS).get(0);
		Utils.defaultWait();
		String label = activeElement.getText();
		...
		while (!found) {
			...
			if (Utils.isThisStringListContainsThisAttribute(Arrays.asList(label), attribute)) {
				...
				activeElement.click();
				Utils.defaultWait();
				// If we did not find it, we go down
			} else {
				index++;
				Utils.switchToActiveElement().sendKeys(Keys.DOWN);
				//activeElement = activeElement.findElement(By.xpath(".//../.."));
				//System.out.println(activeElement.getAttribute("id"));
				//activeElement.sendKeys(Keys.DOWN);
				activeElement = Utils.createGeneralWebElementsFromEnum(SortLocators.SORTER_MENU_OPTIONS).get(index);
