...
public class ViewVersionsManager {
	...
	final int rowNumbers = GeneralTableManager.getAllRowsOfThisGrid(ViewVersionsLocators.VERSIONS_WINDOW_GRIDPANEL)
		.size();
	closeVersionsWindow();
	...
	openVersionsWindow();
	Utils.waitForElementPresent(ViewVersionsLocators.ALL_ROWS);
	final WebElement firstRow = GeneralTableManager
		.getAllRowsOfThisGrid(ViewVersionsLocators.VERSIONS_WINDOW_GRIDPANEL).get(0);

	// Searches for the versionID column, and get value of first row
	final int columnPositionOfVersionID = GeneralTableManager.getColPos(VersionsTableLocators.VERSION_ID,
		ViewVersionsLocators.VERSIONS_WINDOW_GRIDPANEL);
	final int versionID = Integer.parseInt(
		firstRow.findElement(By.xpath("./tbody/tr/td[" + columnPositionOfVersionID + "]/div")).getText());
	closeVersionsWindow();
	...
	final WebElement secondRow = GeneralTableManager
		.getAllRowsOfThisGrid(ViewVersionsLocators.VERSIONS_WINDOW_GRIDPANEL).get(index);
	...
	final List<String> stringValues = GeneralTableManager.getTheseValuesFromThisGrid(
		VersionsTableLocators.VERSION_ID, ViewVersionsLocators.VERSIONS_WINDOW_GRIDPANEL);
