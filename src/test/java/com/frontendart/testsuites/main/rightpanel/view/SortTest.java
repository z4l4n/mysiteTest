...
public class SortTest extends JunitTestClass {
	...
	/**
	 * Simple sort test, clicks on New sorting button
	 * Redmine issue number: <a href="https://redmine.mt2.dsd.sztaki.hu:18018/issues/1128">#1128</a>
	 */
	@Test
	@Category(CoreSuite.class)
	...
	/**
	 * Reimplement: Test sort by clicking on header
	 * Redmine issue number: <a href="https://redmine.mt2.dsd.sztaki.hu:18018/issues/1549">#1549</a>
	 */
	/*@Test
	public final void testSortByClickOnHeader() {
		...
		// TODO: if that header is not visible, we should make it visible.
		...
		for (final WebElement header : GeneralTableManager.getAllVisibleHeadersAsWebElements()) {
			final String label = header.findElement(By.xpath("./div/span/span/span")).getText();
			if (Utils.isThisStringListContainsThisAttribute(Arrays.asList(label), randomLocator)) {
				LOGGER.info("Kattintsunk a " + Utils.getAllMyLabelsAsString(randomLocator) + " nevű oszlop fejlécére.");
				header.click();
				Utils.defaultWait();
				header.click();
				Utils.defaultWait();
	...
	*/
	...
	public final void testSortBySelectingHeader() {
		...
		for (final WebElement header : GeneralTableManager.getAllVisibleHeadersAsWebElements()) {
			...
			if (Utils.isThisStringListContainsThisAttribute(Arrays.asList(label), randomLocator)) {
				LOGGER.info("Kattintsunk a " + Utils.getAllMyLabelsAsString(randomLocator) + " oszlop fejlécénél lévő legördülő nyílra, és válasszuk a növekvő sorrendezés opciót.");
