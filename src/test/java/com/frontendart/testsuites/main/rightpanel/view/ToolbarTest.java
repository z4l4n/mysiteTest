...
public class ToolbarTest extends JunitTestClass {
	...
	/**
	 * Basic search toolbar test
	 * Redmine issue number: <a href="https://redmine.mt2.dsd.sztaki.hu:18018/issues/1047">#1047</a>
	 * Redmine issue number: <a href="https://redmine.mt2.dsd.sztaki.hu:18018/issues/1276">#1276</a>
	 */
	...
	public final void testPageAfterDeselectAll() {
		...
		Utils.myAssertTrue("A Törlés/Remove gombnak inaktívnak kell lennie.", deleteButton.getAttribute("class").contains("disabled"));
		assertTrue("There is a selected row.", Utils.myAssertEquals("Egyetlen sornak sem szabad kijelölve lennie.", 0,
				selectedRowsCount()));
	}

	/**
	 * Search toolbar page check after clicking on Select All button
	 * Redmine issue number: <a href="https://redmine.mt2.dsd.sztaki.hu:18018/issues/1243">#1243</a>
	 */
	@Test
	@Category(CoreSuite.class)
	public final void testPageAfterSelectAll() {
		...
		if ((Utils.getActualRole().equals(Roles.AUTHOR)) && recordType.equals(GeneralRecordTypes.AUTHOR) && (expected > 100)) {
			expected = 100;
		}
		assertTrue("Not all items are selected.", Utils.myAssertEquals("Az összes elemnek kijelölve kell lennie.",
				expected, selectedRowsCount()));
	}
	...
	public final void testNewButtonForCreatableRecordTypes() {
		...
		// Validation
		assertFalse("The new button should be enabled!",
				isThisElementDisabled(ToolbarLocators.NEW_BUTTON));
...
//		final WebElement editorWindow = Utils.switchToActiveElement();
		final WebElement editorWindow = Utils.createGeneralWebElementFromString("//div[starts-with(@aria-labelledby, 'modeleditorwindow')]");
		
		final String textOfWindow = editorWindow.findElement(By.xpath("./div/div/div/div/div")).getText();
		
		
		Utils.myAssertTrue("A megjelenő felugró ablak fejlécének tartalmaznia kell az \"Új hozzáadása\" / \"Add new\"szöveget.",
	...
	public final void testNewButtonForNotCreatableRecordTypes() {
		...
		assertFalse("The new button either should not be visible or it should be disabled!",
				(isThisElementEnabled(ToolbarLocators.NEW_BUTTON) && Utils.isThisElementVisible(ToolbarLocators.NEW_BUTTON)));
	}
	 ...
	 */
	/*	@Test
		public final void testUpdateButtonLabel() {
			// Select random record type
			final GeneralRecordTypes recordType = RecordSelectionManager.selectRandomRecordTypeFromSelector();
			RecordSelectionManager.selectRandomRenderedRowsFromGridPanel(1);

			// Get edit button label
			final String editButtonLabel = Utils.getNameOfThisButton(Utils.createGeneralWebElementFromEnum(ToolbarLocators.EDIT_BUTTON));

			// Validate: Check edit button label status depending on role
			if (Utils.getActualRole().getEditableRecordTypes().contains(recordType)) {
				assertTrue("The label of the edit button is not correct!", editButtonLabel.equals("Edit") || editButtonLabel.equals("Szerkeszt"));
			} else {
				assertTrue("The label of the edit button is not correct!", editButtonLabel.equals("View") || editButtonLabel.equals("Megtekint"));
			}
	 ...
	 */
	/*@Test
	public final void testUpdateEditor() {
		// Select random record type
		final GeneralRecordTypes recordType = RecordSelectionManager.selectRandomRecordTypeFromSelector();
		RecordSelectionManager.selectRandomRenderedRowsFromGridPanel(1);
		...
		Utils.waitForElementPresent(RecordEditorLocators.EDITOR_FORM_FIELDS_VISIBLE_LABELS);
		assertTrue("The labels should be present on the editor!", RecordEditorLocators.checkAttributesAreVisible(recordType.getAttributes()));

		// Validate: Check there is no save button present if we have no right
		if (Utils.getActualRole().getEditableRecordTypes().contains(recordType)) {
			assertTrue("There should be a button present with name: " + Utils.getAllMyLabelsAsString(RecordEditorLocators.SAVE_AND_CLOSE),
					Utils.isThisElementVisible(RecordEditorLocators.SAVE_AND_CLOSE));
		} else {
			assertFalse("There should not be a button present with name: " + Utils.getAllMyLabelsAsString(RecordEditorLocators.SAVE_AND_CLOSE),
	...
	*/
	/**
	 * Search toolbar page check after one item is selected.
	 * Redmine issue number: <a href="https://redmine.mt2.dsd.sztaki.hu:18018/issues/1278">#1278</a>
	 */
	@Test
	@Category(CoreSuite.class)
...
//		final List<ToolbarLocators> disabledLocators = Arrays.asList(ToolbarLocators.RESTORE_BUTTON);

		// Validation
//		assertTrue("The restore, and mark as duplum buttons should be disabled!", areTheseElementsDisabled(disabledLocators));
		assertTrue("The restore button should be disabled!", isThisElementDisabled(ToolbarLocators.RESTORE_BUTTON));
	}

	/**
	 * Search toolbar page check after more than one items are selected.
	 * Redmine issue number: <a href="https://redmine.mt2.dsd.sztaki.hu:18018/issues/1426">#1426</a>
	 */
	@Test
	@Category(CoreSuite.class)
	public final void testToolbarWhenMoreThanOneRecordIsSelected() {
		...
		// Create list of disabled buttons
//		final List<ToolbarLocators> disabledLocators = Arrays.asList(ToolbarLocators.VERSIONS_BUTTON,
//				ToolbarLocators.NEW_TICKET_BUTTON, ToolbarLocators.DUPLICATE_BUTTON);

		final List<ToolbarLocators> disabledLocators = Arrays.asList(ToolbarLocators.VERSIONS_BUTTON,
				ToolbarLocators.NEW_TICKET_BUTTON);
...
//				areTheseElementsDisabled(disabledLocators));
		...
		assertTrue("The new ticket button should be disabled.", isThisElementDisabled(ToolbarLocators.NEW_TICKET_BUTTON));
	}
	...
	public final void testToolbarCitationsButton() {
		...
		// Check citations button is present only if publication record type is selected.
		if (recordType.equals(GeneralRecordTypes.PUBLICATION)) {
			assertTrue("Citations button should be present at this record type: " + RecordSelectionManager.getAllMyLabelsAsString(recordType),
					Utils.isThisElementVisible(ToolbarLocators.CITATIONS_BUTTON));
		} else {
			assertFalse("Citations button should not be present at this record type: " + RecordSelectionManager.getAllMyLabelsAsString(recordType),
					Utils.isThisElementVisible(ToolbarLocators.CITATIONS_BUTTON));
		}
	}

//	/**
//	 * Search toolbar page - import button is present only if publication type is selected.
//	 */
//	@Test
//	@Category(CoreSuite.class)
...
//		final List<GeneralRecordTypes> importVisibleRecordTypes = Arrays.asList(GeneralRecordTypes.PUBLICATION, GeneralRecordTypes.AUTHOR, GeneralRecordTypes.INSTITUTE);
//
//		// import button is present only if publication record type is selected.
//		if (importVisibleRecordTypes.contains(recordType)) {
//			assertTrue("Import button should be present at this record type: " + RecordSelectionManager.getAllMyLabelsAsString(recordType),
//					Utils.isThisElementVisible(ToolbarLocators.IMPORT_BUTTON));
//		} else {
//			assertFalse("Import button should not be present at this record type: " + RecordSelectionManager.getAllMyLabelsAsString(recordType),
//					Utils.isThisElementVisible(ToolbarLocators.IMPORT_BUTTON));
//		}
//	}
	 ...
	 * Redmine issue number: <a href="https://redmine.mt2.dsd.sztaki.hu:18018/issues/1629">#1629</a>
	 * 
	 * TODO: reimplement
	 */
	...
	/**
	 * Search toolbar page check after clicking on Select All button
	 * Redmine issue number: <a href="https://redmine.mt2.dsd.sztaki.hu:18018/issues/1345">#1345</a>
	 */
